...
impl HashTable {
    ...
    pub fn get_offset_mut(&mut self, i: u32) -> usize {
        ...
        let chk = (i >> 24) as u8;
        let mut i = (i.wrapping_mul(16) & self.0.len() as u32 - 16) as usize;
        if self.0[i] == chk {
            i
        } else if self.0[i ^ 16] == chk {
            i ^ 16
        } else if self.0[i ^ 16 * 2] == chk {
            i ^ 16 * 2
        } else {
            if self.0[i + 1] > self.0[i + 1 ^ 16] || self.0[i + 1] > self.0[i + 1 ^ 16 * 2] {
                i ^= 16;
            }
            if self.0[i + 1] > self.0[i + 1 ^ 16 ^ 16 * 2] {
                i ^= 16 ^ 16 * 2
            }
            self.0[i] = chk;
            i
...
const fn compute_statemap_dt() -> [i32; 1024] {
    let mut table = [0; 1024];
    let mut i = 0;
    // `while` loops are well-supported in const contexts.
    while i < table.len() {
        table[i] = (16384 / (i + i + 3)) as i32;
        i += 1;
    }
    table
}
const STATEMAP_DT: [i32; 1024] = compute_statemap_dt();
struct StateMap {
    cxt: usize,
    t: Vec<u32>,
}
...
impl StateMap {
    ...
    fn update(&mut self, bit: u32, limit: u32) {
        ...
        *p = p0.wrapping_add(
            (((((bit << 22) as i32 - prediction) >> 3) * STATEMAP_DT[count as usize])
                & MASK) as u32,
        );
    }
}
...
pub struct Predictor {
    t0: Vec<u8>, // order 1 context --> state
    t: HashTable,
...
impl Predictor {
    pub fn new() -> Predictor {
        let t0 = vec![0u8; 65536];
        let sms = array::from_fn(|_| StateMap::new(256));
    ...
    pub fn update(&mut self, y: u32) {
        ...
        for i in 0..NUM_HIGHER_ORDER {
            self.t.0[self.cps[i]] = STATE_TABLE[self.t.0[self.cps[i]] as usize][y as usize];
        }
        self.m.update(y);
        ...
        for i in 0..NUM_HIGHER_ORDER {
            self.m.add(
                self.stretch
                    .stretch(self.sms[i].p(y, self.t.0[self.cps[i]] as usize)),
            );
        }
